 ...
 * <P>
 * La classe est abstraite car il faut connaitre la tarification et les durees
 * de pret fixees dans les sous-classes.
 */
public class Client implements Serializable {
	...
	/**
	 * Attributs pour les abonnes ces attributs peuvent etre utilises par tous
	 * les clients
	 */
	...
	public Client(String nom, String prenom, String adresse, CategorieClient catClient) throws OperationImpossible {
		...
		if (catClient.getCodeReducUtilise()) {
			throw new OperationImpossible(
					"Call with client type " + this.catClient.getNom() + " and no reduction code");
		}
	}
	...
	/**
	 * Constructeur de client permettant d'initialiser seulement le nom et
	 * prenom
	 *
	 * @param nom
	 *            Nom du client
	...
	private void initAttr(String nom, String prenom, String adresse, CategorieClient catClient)
		...
		if (nom == null || prenom == null || adresse == null || catClient == null) {
			throw new OperationImpossible("Parametre null dans constructeur client : nom =" + nom + " prenom =" + prenom
					+ " " + " adresse= " + adresse + " categorie = " + catClient);
		}
		this.nom = nom;
		this.prenom = prenom;
	 ...
	 * tester l'egalite de deux clients. Il y a egalite quand les noms et
	 * prenoms sont les memes.
	 *
	 * @param obj
	 *            Operande droit
	...
	/**
	 * Rewrite hashCode this will be never used because we hash the key and not
	 * the value
	 *
	...
	/**
	 * <TT>aDesEmpruntsEnCours</TT> verifie si le client a des emprunts en
	 * cours, auquel cas la fonction retourne vrai, faux sinon.
	 *
	...
	/**
	 * <TT>peutEmprunter</TT> verifie si le client peut emprunter le document.
	 * Il peut le faire s'il n'a pas de document en retard de restitution ou
	 * s'il n'a pas atteint son nombre maximal d'emprunt. Si ce n'est pas le
	 * cas, elle retourne false.
	 * <P>
	...
	/**
	 * <TT>emprunter</TT> teste si le client peut emprunter le document et leve
	 * une exception AssertionError si ce n'est pas le cas
	 *
	 * @see #peutEmprunter()
	 */
	...
	/**
	 * <TT>marquer</TT> interdit tout nouvel emprunt par le client. Cette
	 * fonction est appelee par <TT>verifier</TT> de la classe Emprunt.
	 *
	...
	/**
	 * <TT>restituer</TT> est appelee lors de la restitution d'un document
	 * emprunte. S'il s'agissait d'un emprunt en retard les mises a jour sont
	 * alors effectuees.
	 *
	 * @param emprunt
	 *            fiche d'emprunt associee correspondante
	 */
	 ...
	 *
	 * @param enRetard
	 *            Indique si l'emprunt est marque en retard
	 */
	 ...
	 *
	 * @param jour
	 *            Date du pret (date du jour)
	 * @param duree
	 *            Nombre de jours du pret
	...
	/**
	 * <TT>sommeDue</TT> permet de connaitre le tarif d'emprunt d'un document
	 * selon le type de ce document et le type de client. Le tarif pour un
	 * client a tarif normal est le tarif nominal, mais on se reserve la
	 * possibilite d'evolution. On suppose que le reglement est forcement
	 * effectue.
	 *
	 * @param tarif
	 *            Tarif nominal de l'emprunt du document
	 ...
	 */
	public void setCategorie(CategorieClient nCat) throws OperationImpossible {
	 ...
	 * Modifie la categorie du client
	 */
	public void setCategorie(CategorieClient nCat, int reduc) throws OperationImpossible {
		if (nCat.getCodeReducUtilise()) {
